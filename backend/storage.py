...
def init_db():
    ...
    """)
    cur.execute("CREATE INDEX IF NOT EXISTS idx_history_created ON history(session_id, created_at)")
    conn.commit()
    conn.close()
...
def save_record(session_id,record):
    ...
    VALUES (?, ?, ?, ?, ?, ?)
    """, [session_id, record["text"], record["score"], record["label"], record["pinyin"], record["created_at"]])
    conn.commit()
    conn.close()
